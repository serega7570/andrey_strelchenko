...
class AppModel {
  static async getTasklists() {
    ...
    static async addTasklist(tasklistName, tasklistTime) {
        console.log(JSON.stringify({ tasklistName, tasklistTime }));
    const result = await fetch(
      'http://localhost:4321/tasklists',
      {
        ...
        body: JSON.stringify({ tasklistName, tasklistTime })
      }
    );
...
class App {
  ...
  onInputKeydown = async ({ key, target }) => {
    if (key === 'Enter') {
      if (target.value) {
        await AppModel.addTasklist(target.value, time.value);
  ...
  async init() {
    ...
    tasklists.forEach(({ tasklistName, tasklistTime, tasks }) => {
...
class Tasklist {
  ...
  onAddTaskButtonClick = async () => {
    const newTaskName = prompt('Введите название упражнения');
  ...
  onEditTask = async (taskID) => {
    ...
    const newTaskName = prompt('Введите новое название упражнения', oldTaskName);
  ...
  onDeleteTaskButtonClick = async (taskID) => {
    ...
    if (!confirm(`Упражнение '${taskName}' будет удалено. Продолжить?`)) return;
  ...
  render() {
    ...
    header.innerHTML = 'Продолжительность:<br>' + this.tlName;
    tasklist.appendChild(header);

    const time = document.createElement('header');
    time.classList.add('tm-tasklist-header');
    time.innerHTML = 'Описание тренировки:<br>' + this.tlTime;
    ...
    button.innerHTML = 'Добавить упражнение';
